...
if "selected_study" in st.session_state and st.session_state.selected_study in SUBJECTS:
    ...
    # === 教科書のみ表示（ワーク非表示）・全ジャンル横並び ===
    # 登録済み教科書を全ジャンル分収集
    _tb_list = []
    for _gk in genre_keys:
        _td = load_textbook(skey, _gk)
        if _td:
            _tb_list.append((_gk, sinfo["genres"][_gk], _td))

    if not _tb_list:
        st.markdown('<div class="cover-ph">📖 教科書未登録</div>', unsafe_allow_html=True)
    else:
        _tb_cols = st.columns(len(_tb_list)) if len(_tb_list) > 1 else [st.columns([1,2,1])[1]]
        for _ci, (_gk, _ginfo, _td) in enumerate(_tb_list):
            with (_tb_cols[_ci] if len(_tb_list) > 1 else _tb_cols[0]):
                # 表紙画像（GitHub Raw から取得）
                _cover_url = f"https://raw.githubusercontent.com/rebale-minobe/RIA/main/data/textbook_covers/{skey}_{_gk}.jpg"
                try:
                    import requests as _req
                    _cr = _req.get(_cover_url, timeout=5)
                    if _cr.status_code == 200:
                        _b64c = base64.b64encode(_cr.content).decode()
                        st.markdown(
                            f"<div style='text-align:center;padding:8px 0 4px;'>"
                            f"<img src='data:image/jpeg;base64,{_b64c}' "
                        ...
                        st.markdown(
                            f"<div style='background:linear-gradient(135deg,#e0e0e0,#c0c0c0);"
                            f"height:160px;border-radius:10px;display:flex;align-items:center;"
                            f"justify-content:center;color:#888;font-size:14px;margin-bottom:4px;'>"
                            f"📖 {_ginfo['name']}</div>", unsafe_allow_html=True
                        )
                except Exception:
                    pass
                st.markdown(
                    f"<div style='text-align:center;font-size:14px;font-weight:700;"
                    f"color:#1c1c1e;margin:4px 0 8px;'>{_ginfo['emoji']} {_ginfo['name']}</div>",
                    unsafe_allow_html=True
                )
                if st.button("📋 目次", key=f"open_tb_{skey}_{_gk}", use_container_width=True):
                    _open = st.session_state.get("top_tb_open")
                    st.session_state["top_tb_open"] = None if _open == f"{skey}_{_gk}" else f"{skey}_{_gk}"
                    st.rerun()

        # 選択された目次を表示
        _open_key = st.session_state.get("top_tb_open")
        if _open_key and _open_key.startswith(f"{skey}_"):
            _open_gk = _open_key.replace(f"{skey}_", "", 1)
            _open_td = next((_td for _gk, _gi, _td in _tb_list if _gk == _open_gk), None)
            if _open_td:
                _open_gi = sinfo["genres"].get(_open_gk, {})
                st.markdown(f"**{_open_gi.get('emoji','')} {_open_gi.get('name','')} 教科書 目次**")
                for _ch in _open_td["textbook"]["chapters"]:
                    _ch_label = f"{_ch.get('chapter_number','')} {_ch['title']}".strip()
                    with st.expander(_ch_label, expanded=False):
                        for _sec in _ch.get("sections", []):
                            ...
                            for _sub in _sec.get("subsections", []):
                                _pg = f"　p.{_sub['page']}" if _sub.get("page") else ""
                                st.markdown(f"　・{_sub['title']}{_pg}")

    # ワークは非表示（各教科ページで対応）
    gkey = genre_keys[0]  # 後続のdetail_type処理用に設定
